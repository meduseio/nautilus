    ...
    module Configuration
      class Base
        ...
        def parse_port_option(parser)
          parser.on("-p PORT", "--portt PORT", "Enable validator") { |_port| @port = _port.to_u32 }
        end

        def parse_config_option(parser)
          parser.on("-c CONFIG_FILE", "--config=CONFIG_FILE", "Select the network") { |_config| @config = _config }
        end
